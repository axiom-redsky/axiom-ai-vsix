...
>
> 최종 갱신: 2026-07-16 (③ locate 1차 개선 **커밋 완료** — 54c2eb0 ② 문서 일단락 · 2843dcf ③ 관측
> 도구 · f34b0f2 수정①②+테스트. eval:region 85%→**88%**)
>
...
> 타입검증 통과 완주 / 구역 없는 쿼리는 "0"→되묻기 정당 유지). 다음 = 커밋(사용자 결정) →
> 잔여 후보: 관찰 1호(되묻기 라벨 관련도 정렬) · 관찰 3호(되묻기 예문 중복, 외관) ·
> 의미론 연관성(tsserver/LSP — attr-readonly는 국소 버그로 판명, 증거 채집 계속).**
...
| ③ | locate/ — 위치찾기 | 🔶 **1차 개선 완료·커밋(2026-07-16, 2843dcf·f34b0f2)** — 0단계 베이스라인→0.5단계 관측도구(테스트 페이지+SVG)→1단계 수정①(isContentAnchor 화살표 오인)+수정②(disambiguation 확정 무시, 실기기 완주 검증 ✅). **eval:region 85%→88%(36/41), 실측 실패 0**, region-edit 241/0. 잔여=관찰 1호(되묻기 라벨 정렬)·관찰 3호(예문 중복)·의미론 연관성(증거 채집 중) |
| ④ | contracts/ — 설명서 삽입 | ⬜ |
...
- [ ] 1단계-수정③(후보): 관찰 1호 — 되묻기 라벨 관련도 정렬(`slice(0,8)`이 파일 순서 앞 8개만
  노출하는 문제). 착수 여부·시점 사용자 결정.
