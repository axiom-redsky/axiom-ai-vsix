...
>
> 최종 갱신: 2026-07-16 (③ **수정③ 되묻기 라벨 관련도 정렬 완료·미커밋** — region-edit **243/0** ·
> eval:region 88% 유지·회귀 0. 앞선 수정①②는 커밋 완료: 2843dcf·f34b0f2)
>
...
> 보존+"외 56개" / 되묻기→"평가관리의 …" 재요청→pick "1"→적용+타입검증 완주).
> 다음 = 커밋(사용자 결정) → 잔여 후보: 관찰 3호(되묻기 예문 중복, 외관) · 의미론 연관성
> (tsserver/LSP — attr-readonly는 국소 버그로 판명, 증거 채집 계속) · ④ contracts 착수.**
...
| ③ | locate/ — 위치찾기 | 🔶 **1차 개선 완료(수정①②=커밋 2843dcf·f34b0f2, 수정③=미커밋)** — 0단계 베이스라인→0.5단계 관측도구(테스트 페이지+SVG)→1단계 수정①(isContentAnchor 화살표 오인)+수정②(disambiguation 확정 무시, 실기기 완주 검증 ✅)+수정③(되묻기 라벨 관련도 정렬, 관찰 1호). **eval:region 85%→88%(36/41), 실측 실패 0**, region-edit 243/0. 잔여=관찰 3호(예문 중복)·의미론 연관성(증거 채집 중) |
| ④ | contracts/ — 설명서 삽입 | ⬜ |
...
- [x] **1단계-수정③: 되묻기 라벨 관련도 정렬 (2026-07-16, 미커밋)** — 관찰 1호 처방("관련도
  ...
  - **리로드 실사용 확인 ✅ (2026-07-16, 실기기 BigFile 11K줄 × qwen3-coder-64k)**:
    ...
    (신호 쿼리의 정렬 첫째 라벨은 합성 회귀 테스트로 고정 — 테스트 페이지 칩 확인은 보조.)
    커밋은 사용자 결정.
