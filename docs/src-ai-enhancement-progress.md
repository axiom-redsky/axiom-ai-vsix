...
> patch-grounded 32/0 · api-binding 69/0. 다음 = 리로드 실사용 확인 → 커밋(사용자 결정) →
> 잔여 후보: 관찰 3호(되묻기 예문 중복, 외관) · 의미론 연관성(tsserver/LSP — attr-readonly는
> 국소 버그로 판명, 증거 채집 계속) · ④ contracts 착수.**
...
- [x] **1단계-수정③: 되묻기 라벨 관련도 정렬 (2026-07-16, 미커밋)** — 관찰 1호 처방("관련도
  ...
  - 리로드 실사용 확인 대기(3단계 테스트 페이지 또는 실기기 BigFile 무신호/구역명 쿼리로 라벨
    순서 확인 가능). 커밋은 사용자 결정.
