...
> 이후 실기기 채팅 라이브 테스트가 **관찰 2호(disambiguation 확정 무시 — 같은 영역 pick이면
> 재타겟 생략돼 ambiguous에 갇힘)를 채집 → 당일 수정②** 완료(RegionEditService 조건에
> `!loc.safety.ok` 추가 + 회귀 3건).
> 게이트: tsc 0 · compile · **region-edit 241/0** · **eval:region 88%(36/41), 실측 실패 0** ·
> patch-grounded 32/0 · api-binding 69/0 · expectGate 회귀 0 · **VSIX 재패키징 완료**.
> **실기기 재검증 ✅**(BigFile 11K줄: "급여관리의 이메일 입력칸…" → pick "1" → replace 적용 →
> 타입검증 통과 완주 / 구역 없는 쿼리는 "0"→되묻기 정당 유지).
> **수정③(관찰 1호) 완료(미커밋)**: ambiguousCandidates 관련도 정렬(RegionEdit 수집 지점,
> 라벨 토큰 매칭×10+후보 줄 grep 점수 · 무신호 동점=안정 정렬로 종전 파일 순서 보존).
> 게이트: tsc 0 · compile · region-edit **243/0**(신규 2) · eval:region 88% 유지·회귀 0 ·
> patch-grounded 32/0 · api-binding 69/0. **리로드 실사용 확인 ✅**(무신호 되묻기=파일 순서
> 보존+"외 56개" / 되묻기→"평가관리의 …" 재요청→pick "1"→적용+타입검증 완주 / 테스트 페이지
> 되묻기 칩 첫째=평가관리 — 9번째 섹션이 정렬로 승격, 직접 증거). **커밋 완료(dd699de)** →
> ③ locate 1차 개선 일단락. 다음 = ④ contracts 착수(0단계 베이스라인부터) · 잔여 채집:
> 관찰 3호(되묻기 예문 중복, 외관) · 의미론 연관성(tsserver/LSP — 증거 채집 계속).**
...
- [x] **0.5단계 — 관측 도구 구축 (2026-07-16, 커밋 2843dcf)** — locate 층은 단일 export 순수 함수
...
- **[관찰 1호] 되묻기 라벨 폭주 (2026-07-16, 첫 실측 → 당일 정정)**: 테스트 페이지에서
  `ambiguousCandidates`가 60개+ 쏟아짐(부서관리~설정관리 전 섹션). **정정(코드 확인)**: 운영 채팅
  표시는 이미 `SHOW=8` 캡+"외 N개" 요약이 구현돼 있음(ChatViewProvider.ts 되묻기 렌더) — 60개+는
  계기판이 원시 데이터 전체를 보여준 것(의도된 동작). **실제 갭 = 정렬 부재**: `slice(0,8)`이
  파일 위쪽 8개를 관련도 무관하게 자름 → 정답 구역이 파일 뒤쪽이면 선택지에 안 보임. 처방 후보가
  "캡+정렬"에서 **"관련도 정렬만"**으로 축소(예: 앵커 점수 보유 후보 줄의 섹션 우선). 수집 자체
  (RegionEdit)는 무제한 유지가 옳음 — 소비자(채팅/모델/계기판)가 각자 자름. ② 카드 "레버2"와
  같은 지점. 1단계 후보 선택 시 attr-readonly 진단과 우선순위 비교 판단.
  → **수정③으로 처방 적용 완료(2026-07-16)** — 아래 작업 로그 참조.
- **[관찰 2호] disambiguation 확정 무시 (2026-07-16, 실기기 채팅 라이브 → 당일 수정 완료)**:
  구역 명시 쿼리("급여관리의 이메일 입력칸…")에서 분류·파일검출·chosen(수정① 덕에 급여관리
  정확)·모델 pick("1")까지 전부 정상인데 **마지막 재타겟만 생략**되어 되묻기로 샘 — pick이
  채택 영역과 같으면 재실행 안 하던 조건이 ambiguous 상태에선 "확정 버리기"가 됨. 수정②로 해소.
  부수 확인: 구역 없는 쿼리에서 모델이 정직하게 "0"(불확실) → 되묻기 정당 작동.
- **[관찰 3호] 되묻기 예문 중복 (2026-07-16, 외관만·비차단)**: 되묻기 안내의 예문이 원 쿼리
  앞에 후보1 라벨을 기계적으로 붙여 "부서관리의 급여관리의 이메일…"처럼 어색해짐(쿼리에 이미
  구역명이 있는 경우). 동작 무해(수정②로 이 경로 자체가 줄어듦) — 데이터 더 모아 판단.

- [x] **1단계-진단: attr-readonly 재현·원인 격리 완료 (2026-07-16)** — 결론: **국소 결정론 버그**
  (토큰 매칭의 구조 한계 아님). 수정 미착수(진단만, 사용자 결정 대기).
  - 재현(스크래치 프로브, locateEditRegion 직접 호출): 픽스처×"이메일 입력칸을 읽기 전용으로
    만들어줘" → gate=anchor-import, bestLine=4(import 줄), matched=['input']. 후보 목록엔
    "이메일(214~223)"이 #2로 정확히 들어 있음 — **locate의 후보 추리기는 정상, 채택만 실패**.
  - 원인 사슬(②.7 루프 복제 계측으로 확정): ① 유일한 진짜 앵커=이메일@215(label JSX 텍스트,
    df=1) ② 브리지 토큰 'input'이 375줄 `onChange={(e) => setNewSkillInput(…)}`에 매칭
    ③ **isContentAnchor의 JSX 텍스트 판정(`lastIndexOf('>') > lastIndexOf('<')`)이 화살표 함수
    `=>`의 `>`를 태그 닫힘으로 오인** → identifier 속 'input'이 가짜 콘텐츠 앵커 승격
    ④ qual이 [215줄, 375줄] 2개로 흩어짐 → ②.7 수렴 조건 실패(모호 보존 설계) → 구제 포기
    ⑤ 폴백 bestLine=scored[0]=4줄(import) → anchor-import 게이트 → full.
  - **일반성**: 케이스 특유 아님 — 컨트롤 브리지 토큰(input/button/select…)+화살표 핸들러에 그
    identifier(setXxxInput·handleButtonClick류)가 있는 모든 파일에서 같은 흩어짐 발생 가능.
    속성 토글류(읽기전용/비활성/필수) 편집 유형의 공통 약점 지대.
  - **처방안(검증 완료·미적용)**: isContentAnchor에서 `before.replace(/=>/g,'  ')` 후 비교 —
    화살표 `>`만 제외. 시뮬레이션 7케이스(범인 제거·정상 앵커 4종 보존·비앵커 2종 유지) 전부
    기대 일치. 적용 시 qual=[215] 수렴 → ②.7 구제 → 기대 효과 eval:region 85%→88%(36/41).
    게이트: test:region-edit(SCATTER 보존 단언 포함)+eval:region 회귀 0 필수.
  - 의미론 연관성(tsserver/LSP) 트랙 판단 재료: 이 건은 **국소 버그라 착수 근거 아님** —
    토큰 매칭 자체는 이 케이스에서 정답 후보를 추렸음(후보 #2). 근본 한계 증거는 계속 채집.
- [x] **1단계-수정①: isContentAnchor 화살표 오인 수정 적용 (2026-07-16, 커밋 f34b0f2)** —
  `RegionEdit.ts` isContentAnchor에 `before.replace(/=>/g,'  ')` 한 줄(+주석): 화살표 `=>`의
  `>`를 JSX 텍스트 판정에서 제외. 회귀 테스트 신규 1건(test-region-edit "화살표 핸들러
  identifier는 콘텐츠 앵커 아님 → 유일 라벨(이메일)로 ok" — 수정 전 실패 구조).
  - 게이트: tsc 0 · compile OK · **region-edit 238/0(신규 1 포함, SCATTER 보존 단언 무회귀)** ·
    **eval:region 85%→88%(36/41)** — attr-readonly ok 전환(anchor-import 3→2), expectGate 전부
    일치(회귀 0). 잔여 비적격 5건은 전부 정당 가드(합성 대조군 3+없는기능 2) — **실측 실패 0**.
  - 리로드 실사용 확인 대기(3단계 테스트 페이지에서 유사 쿼리 재확인 가능). 커밋은 사용자 결정.
- [x] **1단계-수정②: disambiguation 확정 무시 수정 (2026-07-16, 커밋 f34b0f2)** — 실기기 채팅 라이브
  테스트(BigFile × "급여관리의 이메일 입력칸…")가 채집한 **관찰 2호** 버그의 당일 수정.
  - 원인: RegionEditService 재타겟 조건이 `pick이 채택 영역과 다를 때만` locateEditRegion(forced)
    재실행 — 모델이 휴리스틱 채택과 **같은** 영역을 골라 "확정"해준 경우(pick="1"=후보1),
    ambiguous 게이트가 그대로 남아 정답 pick을 버리고 되묻기로 샜다(라이브 로그: 모델 선택
    "급여관리(2431~2494)" 직후 "게이트 차단(ambiguous)").
  - 수정: 조건에 `!loc.safety.ok` 추가 — 게이트 막힌 상태의 동일-영역 pick도 forcedRegion
    재실행으로 게이트 우회(모델 "0"=불확실(null)은 종전대로 되묻기 유지 — 라이브 테스트 1에서
    정직한 "0" 확인됨).
  - 회귀 테스트 신규 3건(test-region-edit): 합성 diffuse 파일(5섹션×280pad, spread>1000) 전제
    ambiguous 확인 + 동일-영역 확정 pick→applied + null→되묻기 유지.
  - 게이트: tsc 0 · compile OK · **region-edit 241/0** · eval:region 88% 유지(회귀 0,
    eval은 disambiguate 미주입이라 종전 동일) · patch-grounded 32/0 · api-binding 69/0 ·
    VSIX 재패키징 완료.
  - **실기기 재검증 ✅ (2026-07-16, qwen3-coder-64k × BigFile 11K줄)**: ① 구역 없는 쿼리 →
    모델 "0"(불확실) → 되묻기 유지(정당) ② "급여관리의 이메일 입력칸…" → pick "1" →
    forcedRegion 재진입 → **anchor-first replace 적용(원본 2455~2460줄) → Stage 0 타입검증
    통과 → 수정 대기 diff**, 프롬프트 ≈4,197토큰(예산 17%). 11K줄 파일에서 분류→파일검출→
    chosen→모델 pick→재타겟→적용→검증 전 사슬 완주 — 되묻기 두더지 해소 확인.
- [x] **1단계-수정③: 되묻기 라벨 관련도 정렬 (2026-07-16, 커밋 dd699de)** — 관찰 1호 처방("관련도
  정렬만") 적용. 수집 지점(RegionEdit ambiguousCandidates, 단일 진실원)에서 관련도 내림차순
  정렬 — 소비자 캡(채팅 `slice(0,8)`)은 그대로, 계기판(3단계 테스트 페이지)도 같은 순서를 봄.
  - 관련도 = **라벨 텍스트의 질문 토큰 매칭(dedupeSubstrings, ×10) + 그 라벨에 속한 후보 줄의
    최고 grep 점수**. 전부 동점(무신호 쿼리)이면 안정 정렬이라 종전 파일 순서 그대로(회귀 0).
    부수 효과: 되묻기 예문(`cands[0]`)도 가장 관련도 높은 라벨로 바뀜(관찰 3호 어색함 일부 완화).
  - 회귀 테스트 신규 2건(test-region-edit, 관찰 2호 합성 diffuse 픽스처 재사용): ① 파일 맨 아래
    (5번째) 섹션을 지목한 쿼리("자격증관리의 입력칸…")에서 그 라벨이 첫째 ② 무신호 쿼리는
    종전 파일 순서 유지(부서관리 첫째·자격증관리 다섯째).
  - 게이트: tsc 0 · compile OK · **region-edit 243/0(신규 2)** · **eval:region 88% 유지(expectGate
    전부 일치, 회귀 0)** · patch-grounded 32/0 · api-binding 69/0.
  - **리로드 실사용 확인 ✅ (2026-07-16, 실기기 BigFile 11K줄 × qwen3-coder-64k)**:
    ① 무신호 쿼리("입력칸을 읽기 전용으로") → 모델 정직 "0" → 되묻기 라벨 8개 **종전 파일 순서
    그대로**(부서관리~자격증관리)+"외 56개" 캡+예문=첫 라벨 — 무신호 보존 라이브 확인.
    ② 되묻기 안내대로 구역명 재요청("평가관리의 입력칸…", 평가관리=파일 9번째 섹션=캡 8개 밖) →
    콘텐츠 앵커 채택+후보1=평가관리 → 모델 pick "1" → 수정② 게이트 우회 → **replace 적용
    (2899~2904)+Stage 0 타입검증 통과**, ≈4,218토큰(17%) — "되묻기→구역명 재요청→완주" 루프 실증.
    ③ 3단계 테스트 페이지: 무신호 되묻기 라벨 파일 순서·ambiguous 게이트·후보 표 정상 렌더 +
    **신호 쿼리("평가관리의 …")에서 되묻기 칩 첫째=평가관리 확인 — 파일 9번째 섹션이 정렬로
    승격된 직접 증거**(합성 회귀 테스트와 일치). 사용자 커밋 완료(dd699de).
  - 관찰(비차단): 무신호 되묻기에선 정렬 근거 자체가 없어 캡 8개 밖 섹션(평가관리 등 56개)은
    여전히 선택지에 안 보임 — 설계상 정상(정렬은 신호 있을 때만 유효). 사용자가 화면을 보고
    구역명을 아는 실사용에선 문제 안 됨. 갭이 실측되면 "외 N개 전체 보기" UI류는 별도 후보.
...
- [x] **리로드 실측 확인 ✅ + 계기판 오탐 1건 수정 (2026-07-16, 커밋 완료)** — 4단계 페이지 ×
  ...
  - ① "직원 테이블에 /api/employees 적용해줘" → **2/13**(use-api: 쿼리+코드deps ·
    ...
    — CLI 자체검증과 동일 결과 재현.
  - ② "데이터 테이블을 스마트테이블로 바꿔줘" → **2/13**(use-api: 쿼리+deps ·
    smart-table-binding: 쿼리) · **파생 신호 2종(patch 강제 + 루트 교체 허용→SmartTable) 칩
    정확 표면화** · 1,019tk(4%).
  - ③ "getArr 결과를 테이블로 보여줘" → **1/13**(use-api deps만) · 370tk(2%) —
    local-data-render 미발동은 **정당**(getArr가 이 파일에 없음 → referencesLocalDataSource
    존재 검사가 막음. 음성 대조 정상). 파일에 없는 심볼 렌더 요청 자체는 ①intent
    "대상 파일 해석 고도화" 소관(포인터만).
  - ④ "입사일을 달력으로 바꿔줘" → **2/13**(use-api: deps · date-picker: 쿼리 "달력") ·
    781tk(3%) — 레시피 카드 발동 정확.
  - **채집 표본 2건(트리거 정밀도 — 기록만, 추가 작업 1번의 첫 데이터)**: ⓐ use-api는 deps
    트리거로 4/4 상시 발동 — useApi 쓰는 파일이면 쿼리 무관 고정비용 504자(정당·무해하나
    상시성 자체가 표본) ⓑ use-api 쿼리 근거가 "데이터" 단어만으로도 발동(②번 실측,
    ScaffoldContracts.ts:139 정규식의 `데이터|목록` — 광폭 트리거 표본. ②는 deps로도 발동하는
    파일이라 실해 0).
  - 주입 비용 실측 **2~5%(370~1,119tk)** — 베이스라인 "주입 비용은 현재 문제 아님" 실측 재확인.
    prop 표는 4/4 동일(영역 없음→파일 전체 스캔: 감지 11종·주입 앞 3개 Card/CardHeader/CardTitle·
    453자≈151tk 고정).
  - **계기판 오탐 수정(패널 전용)**: "인덱스에 없는 PascalCase 태그" 채널이 **T*Response 제네릭
    60여 개로 도배** — 패널 tagRe가 `useApi<TDeptResponse>('/api/…')`의 제네릭 인자를 태그로
    오인(JSX 태그는 식별자 문자 바로 뒤에 올 수 없음). 수정=앞글자 가드(ContractsProbePanel.ts,
    식별자 문자 뒤 `<`는 제외) — 인덱스 공백 신호 채널이 본 목적(재생성·TARGET_FILES 후보)을
    되찾음. ⚠ 운영 detectComponentsInRegion도 같은 정규식이나 **인덱스 필터로 무해**(주입 오염
    0·제네릭 타입은 인덱스에 없음) — 운영 쪽은 미변경(제로리스크), 추가 작업 2번에 노트.
  - 게이트: tsc 0 · compile OK · region-edit 243/0. **재확인 시 리로드 필요**(패널 수정 반영).
...
2. **ComponentPropsIndex 재생성 자동화** — 현재 수동(`npm run build:component-props`) +
   ...
   칩이 공백 실측 데이터를 준다. +실측 노트(2026-07-16): 운영 detectComponentsInRegion의 태그
   정규식도 제네릭 인자를 매칭하나 인덱스 필터로 무해(`useRef<Button>` 류만 이론상 과포함) —
   이 항목 착수 시 패널과 같은 앞글자 가드 이식 검토.
