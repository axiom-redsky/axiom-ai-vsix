...
- **Phase 4 — 확장** (택1로 진행중)
  ...
  - **D2 Mock 데이터 생성기 (2026-09-01): 완료 · F5 라이브 검증 통과**
    ...
    - **F5 실측으로 확인된 것**(수정 사항 없음): 실 프로젝트(nicify)에서 타입 목록이 등급 딱지와 함께 뜨고
      **화면용 61개는 접힌 채** 시작 · `IOrd` 검색 → `IOrder`(데이터)·`IOrderFilters`(데이터)·
      `IOrderDetailDialogProps`(일부)가 등급대로 갈려 나옴 · 제네릭 띠
      `{ success: boolean; data: IOrder[]; … }` · 미리보기에서 **`customer`가 한글 이름**(강서준·조유나·윤지호)·
      `orderNo` `ORDER-0001…` · `phone` `010-0000-0001` · **`status`가 `pending`→`paid`로, `channel`이
      `온라인`→`모바일`로 순환** · [파일로 저장] → `public/mock/order.json` 생성 + 오른쪽 아래 알림 +
      **다시 열었을 때 버튼이 [덮어쓰기]로 바뀜** · 열어 본 파일 내용이 미리보기와 일치.
