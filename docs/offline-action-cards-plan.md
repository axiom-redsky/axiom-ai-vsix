...
> 상태: **Phase 0~3 완료 · Phase 4 진행중(A3 ✅ / C1 ✅ / B1 ✅ / 형태 B ✅ / A4 ✅ / B2 ✅ / B3 ✅ / B4 ✅ / D1 ✅ / D2 코드 완료·F5 대기)** — §7 A·B·C 소진 + D 진행 (2026-09-01, §9 진행표)
...
(**아홉 트랙 전부 F5 라이브 검증 통과**). **D2(Mock 데이터 생성기)는 코드·게이트 완료, F5 대기**.
전 게이트 green. **§7 A·B·C는 전부 소진**됐고 D 그룹을 진행 중이다 — 남은 것은 D3와
§6 채집 플라이휠(설계 먼저).
...
> ⚠ **D2도 파일을 쓴다(두 번째).** 같은 규약을 따르되 **한 가지만 다르다 — 덮어쓰기를 막지 않는다**:
> mock은 건수를 늘리거나 타입이 바뀌면 **다시 만드는 게 정상**이라서다. 대신 ①경로를 `public/mock/`으로
> 고정하고 ②이미 있으면 버튼 이름이 **[덮어쓰기]** 로 바뀌며 ③`WorkspaceEdit` 한 번이라 Ctrl+Z로 되돌아간다.
...
> **D2는 미커밋**(2026-09-01 기준). 이어서 하기 전에 `git status`로 현재 상태를 먼저 확인할 것.

### 다음에 할 일 = 아래 중 택1

| 후보 | 무게 | 시작점 |
|---|---|---|
| **★ D2 F5 수동 검증**(가장 먼저) | 아주 작음 | 코드·게이트는 끝났고 **실 화면 확인만 남았다**. ⚠ 파일을 쓰는 기능이라 확인이 더 중요하다 — 아래 "F5로 눈으로 확인하는 법"의 Mock 데이터 절 |
| **§7 D3 오프라인 큐잉** | 중간 | LLM이 필요한 요청을 로컬 JSONL에 쌓았다가 온라인 복귀 시 일괄 제안. 원료는 `ai/pipeline/RegionCaptureRecorder` |
...
- **퍼블리싱 포팅(D1)** = 온·오프라인 무관·모델 호출 0. ⚠ **파일을 쓰는 유일한 창**이라 확인이 더 중요하다.
  ...
  8. 되돌리지 말고 패널을 닫았다 다시 열어 또 누르면 "이미 있음 · 건너뜀"이 뜨고
     라우트가 **두 줄 생기지 않는지**(멱등).
- **Mock 데이터(D2)** = 온·오프라인 무관·모델 호출 0. ⚠ **파일을 쓰는 두 번째 창**이라 확인이 중요하다.
  런처의 `🧪 Mock 데이터` 또는 명령 팔레트 `Axiom AI: Mock 데이터 열기`.
  창은 **왼쪽 칸(타입 목록)** + **오른쪽 칸**으로 나뉘고, 오른쪽은 위에서부터
  **옵션 줄 → `useApi 제네릭` 띠 → 미리보기 → 화면에서 쓰는 법 → 맨 아래 버튼 3개** 순서다. 그 순서대로:
  1. **왼쪽 타입 목록**: 이름 옆에 **초록 `데이터` · 노랑 `일부` · 회색 `화면`** 딱지가 붙는다.
     딱지에 마우스를 올리면 **그렇게 판단한 이유**가 뜬다. 기본은 `화면`(=prop 타입) 숨김이고
     맨 아래 체크박스로 볼 수 있다 — 실측 nicify는 데이터 80 · 일부 117 · 화면 65다.
     `.tsx` 파일을 열어 두면 **그 파일의 타입이 맨 위**로 오고 파란 `현재 파일` 딱지가 붙는다.
  2. 검색창에 `Order` → **`IOrder`** 를 고른다(`domains/example/components/templates/list/order-data.ts`).
  3. **`useApi 제네릭` 띠**에 `{ success: boolean; data: IOrder[]; totalCount: number }` 가 보이는지.
     ★ 이게 이 기능의 핵심이다 — **아래 JSON과 같은 모양**이어야 한다. 옵션 줄의 `봉투`를
     `감싸지 않음`으로 바꾸면 **띠와 JSON이 함께** 바뀐다(하나만 바뀌면 버그: 붙여넣은 제네릭이 거짓말을 한다).
  4. **미리보기**에서 볼 것: `status`가 행마다 `pending → paid → shipping`으로 **돌아가며** 나오는지
     (전부 같은 값이면 상태별 뱃지·필터를 확인할 수 없다) · `orderNo`가 `ORDER-0001`… ·
     `phone`이 `010-0000-0001` · `amount`가 금액다운 자릿수 · `orderedAt`이 일시.
  5. **결정론**: 시드를 `7`로 바꿨다가 다시 `1`로 되돌리면 **처음과 글자 하나까지 같은 JSON**이어야 한다.
     (다시 만들 때마다 값이 흔들리면 git diff가 통째로 뒤집혀 아무도 다시 만들지 않는다.)
  6. 맨 아래 **[파일로 저장]** → `public/mock/order.json`이 생기고, 오른쪽 아래 알림의 **[파일 열기]**로
     열어 **미리보기와 글자 그대로 같은지** 본다.
  7. **Ctrl+Z 한 번**으로 되돌아가는지.
  8. 되돌리지 말고 다시 저장하면 버튼 이름이 **[덮어쓰기]** 로 바뀌고, 눌러도 파일이 **하나뿐인지**.
     (D1과 달리 mock은 다시 만드는 게 정상이라 덮어쓰기를 막지 않는다 — 대신 밝힌다.)
  9. ★**진짜 확인**: scaffold에서 `npm run dev` 후 브라우저로 `http://localhost:5173/mock/order.json` —
     열리면 `[useApi 코드 복사]`한 두 줄을 페이지에 붙여 **백엔드 없이 표가 뜬다**(이 기능의 목적).
  10. 타입 파일에 필드를 하나 추가해 **저장**하면 목록·미리보기가 따라 바뀌는지.
...
- **Phase 4 — 확장** (택1로 진행중)
  ...
  - **D1 퍼블리싱 포팅 (2026-09-01): 완료 · F5 라이브 검증 통과**
    ...
    - **F5 실측으로 확인된 것**(수정 사항 없음): 계획 화면(옮길 화면 1 · 새 파일 2 · 고칠 파일 1) →
      [이대로 옮기기] → `src/domains/example/pages/ExamplePage.tsx`가 생기고 **첫 줄이
      `@/domains/example/components/ExampleCard`로 재작성됨** · 도메인 라우터에
      `path: 'example-page'` · `name: '예제 페이지'` **그대로 승계**(319줄 근처) ·
      "라우터 맵에서 확인" 알림 · **두 번째 실행에서 "옮길 파일이 전부 이미 존재합니다"**(멱등) ·
      `src/publishing/` 원본 유지.
  ...
  - **D2 Mock 데이터 생성기 (2026-09-01): 코드·게이트 완료 · F5 수동 검증 대기**
    §7 D 그룹의 둘째. 계획서가 적어 둔 "**JsonTypeGenerator 역방향**"을 문자 그대로 구현했다.
    - **왜**: 폐쇄망에서 화면은 대개 백엔드보다 먼저 만든다. 타입은 이미 있는데 데이터가 없어서
      JSON을 손으로 찍고, 매번 같은 자리에서 틀린다 — ①필드를 타입과 다르게 적어 화면이 조용히 빈칸이 되고
      ②**봉투를 빠뜨려** `data?.data`가 `undefined`가 되고 ③20건은 노가다라 3건만 만들어
      스크롤·페이징을 못 본다.
    - ★**봉투와 제네릭을 한 값에서 만든다**: scaffold `useApi`는 서버 봉투를 **안 벗긴다**
      (`knowledge/patterns/use-api.md`, 이 계약은 모델도 나도 한 번씩 오판한 자리다). 그래서 mock JSON과
      `useApi<T>` 한 줄을 **같은 계산에서 함께** 내보낸다 — 보이는 JSON과 붙여넣는 제네릭이
      구조적으로 어긋날 수 없다. 이미 봉투 모양인 타입은 **또 감싸지 않는다**(판정은 온라인의
      `detectEnvelopeKey`를 그대로 재사용 — 어휘가 갈라지면 두 층이 다른 말을 한다).
    - **결정론**: 값은 난수 순서가 아니라 `시드 + 필드 경로 + 행 번호`의 해시에서 나온다. 필드 순서가
      바뀌어도 다른 필드가 흔들리지 않고, 같은 옵션이면 언제나 같은 파일이다 — 다시 만들 때 git diff가
      통째로 뒤집히면 아무도 다시 만들지 않는다.
    - **리터럴 유니온은 행마다 돌려 쓴다**: `'대기'|'승인'|'반려'` 인데 20건이 전부 '대기'면 상태별
      뱃지·필터를 확인할 수 없다. 목록 mock의 값어치는 대부분 여기서 나온다(실측 `IOrder.status`가
      `pending→paid→shipping`으로 순환).
    - **저장 위치가 곧 기능**: `public/mock/employee.json` — Vite가 `public/`을 `/`로 서빙하므로
      **백엔드 없이 `useApi('/mock/employee.json')`이 실제로 응답한다**. 그래서 스니펫이 그 주소를 쓴다.
      `public/`은 빌드에 포함되니 연동 후 지우라는 안내를 화면에 적었다.
    - **타입 읽기는 컴파일러 없이**(B3·B4와 같은 결정론 텍스트 파서, 텍스트 도구 `stripComments`·
      `matchBracket`은 B4 것을 **공유**): 주석 처리된 선언 제외 · `import`한 타입 해소 ·
      **제네릭 인스턴스화**(`TApiResponse<TEmployee[]>`) · `extends` 병합 · `Pick/Omit/Partial/Record` ·
      여러 줄 유니온 · 인덱스 시그니처 · 재귀 타입 정지.
    - ⚠ **실 자료가 바로잡은 것 셋**(합성 케이스로는 안 보였다):
      · **프로젝트 타입의 대부분은 데이터가 아니다** — 262개 중 화면 부품 prop 타입이 태반이라
        전부 늘어놓으면 쓸 타입이 묻힌다. 그래서 **데이터/일부/화면 등급**(`mockability`)을 매겨
        기본 목록에서 `화면`을 접었다(숨기지 않고 체크박스로 볼 수 있다 — 판단은 사람이 한다).
        실측 **데이터 80 · 일부 117 · 화면 65**.
      · **"못 찾음"을 뭉뚱그리면 원인이 안 보인다** — 처음엔 134/262가 "타입을 찾지 못함"이었는데,
        열어 보니 거의 전부 **남의 타입**(`React.ReactNode`·`ColumnDef`)이거나 **인자를 안 정한 제네릭 자리**
        (`TRow`)였다. 이름 목록이 아니라 그 파일의 **`import` 표**로 외부 여부를 판정하도록 바꾸고,
        빈 제네릭 자리를 따로 구분하니 **우리 코드에서 진짜 못 찾은 선언은 1개**가 됐다(G3b가 이 선을 고정).
      · **`ColumnDef<TRow>[]`의 `[]`를 못 떼던 버그** — 괄호 균형 검사가 `walkTokens`의 깊이를 빌려 썼는데
        그 콜백은 **괄호가 아닌 글자에서만** 불려서 `>`로 끝나는 표현의 마지막 깊이를 못 봤다.
    - **파일 쓰기는 D1 규약 + 한 가지 완화**: 미리보기 = 저장될 내용, 사람이 누를 때만, `WorkspaceEdit`
      한 번(Ctrl+Z 한 번). 다만 **덮어쓰기는 막지 않는다** — mock은 다시 만드는 게 정상이라서다.
      대신 경로를 `public/mock/`으로 고정하고, 파일이 있으면 버튼 이름이 **[덮어쓰기]** 로 바뀐다.
      ★적용 시점에 **다시 계산한다**(화면의 미리보기는 오래됐을 수 있다 — D1과 같은 규칙).
    - **자료 읽기는 `providers/TypeSource.ts` 한 곳**(B2 `CatalogSource`·B3 `TokenSource`·B4 `RouterSource`와
      같은 규약). 파일 단위 mtime 캐시라 저장한 파일만 다시 읽는다. 편집기에서 보고 있는 파일의 타입은
      목록 맨 위로 올린다(`현재 파일` 딱지).
    - 게이트: `test:mock-data` 129/0(A 선언수집 12 · B 타입식 39 · C 값 22 · D 봉투·제네릭 23 ·
      E 역방향 왕복 18 · F 타입 등급 6 · G 실 자료 9) · typecheck · compile ·
      무회귀(publishing-handoff 47/0 · router-map 63/0 · design-tokens 68/0 · scaffold-hover 76/0 ·
      component-catalog 126/0 · scaffold-lint 64/0 · action-cards 235/0 · offline-recipe 81/0 ·
      offline-api-binding 96/0 · react-rules 39/0 · region-edit 243/0 · offline-intent 73/0 ·
      api-binding 75/0 · knowledge-routing 80/0).
