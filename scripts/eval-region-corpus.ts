 ...
 */
const EMPLOYEE_FORM = [
  // ⚠ 실파일 충실 재현: `import type React`가 named react import보다 **먼저** 온다.
  //   이 선두 type-only 줄이 react 훅(useRef/useEffect) 머지를 깨던 버그의 재현 조건(StructuralAnchor
  //   computeImportAnchor lineIndex0 고정). 픽스처가 이걸 빠뜨려(한 줄 import) 거짓 통과했었다.
  /*  1 */ "import type React from 'react';",
  /*  2 */ "import { useState } from 'react';",
  /*  3 */ "import { Button, Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@axiom/components/ui';",
  /*  4 */ '',
  /*  5 */ 'type TEmployeeForm = { name: string; hireDate: string; department: string };',
  /*  6 */ '',
  /*  7 */ 'export default function EmployeeFormPage(): React.ReactNode {',
  /*  8 */ "  const [name, setName] = useState('');",
  /*  9 */ "  const [hireDate, setHireDate] = useState('');",
  /* 10 */ "  const [department, setDepartment] = useState('');",
  /* 11 */ '',
  /* 12 */ '  const handleSubmit = (): void => {',
  /* 13 */ '    // 등록 처리',
  /* 14 */ '  };',
  /* 15 */ '',
  /* 16 */ '  return (',
  /* 17 */ '    <div className="form-page">',
  /* 18 */ '      <div className="mb-4">',
  /* 19 */ '        <label className="block text-sm font-medium mb-1">이름 *</label>',
  /* 20 */ '        <Input value={name} onChange={(e) => setName(e.target.value)} />',
  /* 21 */ '      </div>',
  /* 22 */ '      <div className="mb-4">',
  /* 23 */ '        <label className="block text-sm font-medium mb-1">입사일 *</label>',
  /* 24 */ '        <Input',
  /* 25 */ '          type="date"',
  /* 26 */ '          value={hireDate}',
  /* 27 */ '          onChange={(e) => setHireDate(e.target.value)}',
  /* 28 */ '          className="h-9 bg-muted/60"',
  /* 29 */ '        />',
  /* 30 */ '      </div>',
  /* 31 */ '      <div className="mb-4">',
  /* 32 */ '        <label className="block text-sm font-medium mb-1">부서</label>',
  /* 33 */ '        <Select value={department} onValueChange={setDepartment}>',
  /* 34 */ '          <SelectTrigger>',
  /* 35 */ '            <SelectValue placeholder="부서" />',
  /* 36 */ '          </SelectTrigger>',
  /* 37 */ '          <SelectContent>',
  /* 38 */ '            <SelectItem value="dev">개발팀</SelectItem>',
  /* 39 */ '            <SelectItem value="hr">인사팀</SelectItem>',
  /* 40 */ '          </SelectContent>',
  /* 41 */ '        </Select>',
  /* 42 */ '      </div>',
  /* 43 */ '      <Button onClick={handleSubmit}>등록</Button>',
  /* 44 */ '    </div>',
  /* 45 */ '  );',
  /* 46 */ '}',
].join('\n');
