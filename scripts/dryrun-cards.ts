 ...
 * 확신도 임계(§10.6) 튜닝·트리거 겹침 디버깅용 계기판.
 *
 * 내장 카드만 본다(워크스페이스가 없는 CLI라 프로젝트·개인 계층은 대상이 아니다).
 * 3계층 전체·상황 필터·전제조건까지 보려면 **관리 패널의 드라이런**(명령 팔레트 →
 * "Axiom AI: 행동 카드 관리")을 쓸 것 — 운영과 같은 매처를 같은 워크스페이스 상황으로 호출한다.
 *
 * 사용:
