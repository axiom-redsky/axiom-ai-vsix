// 오프라인 JSX 이식 단위 테스트: vscode를 빈 객체로 스텁한 뒤 esbuild로 번들 → node 실행
...
const dir = mkdtempSync(join(tmpdir(), 'axiom-transplant-'));
...
await build({
  entryPoints: ['scripts/test-offline-transplant.ts'],
  bundle: true,
  platform: 'node',
  format: 'esm',
  outfile: out,
