...
- keywords: [api, fetch, axios, 통신, 서버, useapi, use-api, query, mutation, get, post, put, delete, patch, 조회, 생성, 수정, 삭제, invalidate, 캐시]
...
- keywords: [router, 라우터, route, 라우팅, navigate, 이동, 경로, path, 해시, hash, createhashrouter, loadable, 코드스플리팅]
  files: [patterns/router.md]

## 구조/규칙

- keywords: [폴더, 구조, 파일, folder, structure, 어디에, 디렉터리, directory, 아키텍처, architecture, 레이어, layer]
  files: [scaffold/project-structure.md, patterns/domain-structure.md]
...
- keywords: [import, 임포트, alias, 앨리어스, 경로, @axiom, @/, 절대경로]
  files: [conventions/naming.md, scaffold/project-structure.md]
